...
import {getMaticClient, getAccount} from "./utils";

async function execute() {
    const { matic, network } = await getMaticClient();
    ...
    // deposit
    await matic.depositEther(token, from, amount).then((res: { transactionHash: any; }) => {
    ...
    await matic.startWithdraw(token, amount2, { from }).then((res: { transactionHash: any; }) => {
    ...
    // 2 confirm-withdraw
    const txHash = "<>";
    await matic.withdraw(txHash, { from, gas: "7000000" }).then((res: { transactionHash: any; }) => {
        console.log("Confirm withdraw hash: ", res.transactionHash);
    });
    
    //Process Exit
    await matic.processExits(token, { from, gas: 7000000 }).then((res: { transactionHash: any; }) => {
